...
% FigureS1_plot.m
%
% Generate Supplementary Figure S1 from
% Gray–Scott simulation data.
%
% Input:
%   record_u
%   record_v
...
figure;
[a1,a2,a3]=size(record_u);
for test=1:a3
    Feed=record_parameters(test,1);
    k=record_parameters(test,2);
    Du_gs=record_parameters(test,3);
    Dv_gs=record_parameters(test,4);

    subplot_er(21,15,round(Du_gs/0.1)+15*round(Dv_gs/0.05));imagesc(record_u(:,:,test));
    colormap(gray);
    % clim([0 1])
...
for test=1:14
    subplot_er(21,15,test);
    imagesc(ones(Nx,Ny));
    colormap(gray);
    clim([0 1])
    set(gca,'XTickLabel',{},'YTickLAbel',{},'Box','off')
    text(0, 100, ['Du=',num2str(test/10),''], 'FontSize', 10, 'Color', 'black');
end
...
for test=[2:21]*15
    ...
    clim([0 1])
    set(gca,'XTickLabel',{},'YTickLAbel',{},'Box','off')
    text(0, 100, ['Dv=',num2str((test-15)/15/20),''], 'FontSize', 10, 'Color', 'black');
end
